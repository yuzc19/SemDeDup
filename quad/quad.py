...
from Quad_util import (
    get_cluster_neighbour,
    get_sample_index,
    load_model,
    sample_minibatch,
    sorted_highest_score,
)
...
cluster_score = {
    i: 0 for i in range(0, cluster_size)
}  # The reward score for each cluster.
cluster_chose = {
    i: 0 for i in range(0, cluster_size)
}  # How many times each cluster has been selected (including selections of neighboring clusters)
cluster_ucb = {
    i: 0 for i in range(0, cluster_size)
}  # The Cluster Score(CS) for each cluster
cluster_sample = {
    i: 0 for i in range(0, cluster_size)
}  # The actual number of times each cluster has been sampled
final_chose = {
    i: 0 for i in range(0, cluster_size)
}  # Which clusters were ultimately selected
    ...
    for i in range(batch):
        ...
        while flag == False:
            if k == 0:
                ...
                loopindex = loopindex + 1
            flag, minibatch = get_sample_index(
                sample_index, real_batchsize, cluster_sample
            )
            if flag == False:
...
def step3(
    ...
    for i_dict in topk:
        for key, value in i_dict.items():
            ...
            cluster_sample[i] += 1
        if (
            averages[idict_index] > 0
        ):  # Cluster has a sufficiently large positive impact on the model's performance.
            final_chose[i] += 1
            final_sum_chose += 1
        ...
        for cluster in cluster_neighbour[i]:  # Update the reward for each cluster
            ...
            cluster_score[cluster] += averages[idict_index] * (
                1 - cluster_distance[(i, cluster)] / threshold
            )
    ...
    for key in cluster_ucb.keys():  # Update the cluster score
        ...
        ucb = alpha * math.sqrt(
            2 * (math.log(float(sum_chose))) / float(cluster_chose[key] + 1)
        )
...
def main():
    ...
    for k in tqdm(range(iteration)):
        ...
        final_sum_chose, sum_average, sum_chose = step3(
            topk,
            cluster_neighbour,
            cluster_distance,
            averages,
    ...
    for i in tqdm(range(cluster_size)):
        if final_chose[i] != 0:
            ...
            # get local data
            with open(
                "../data/cluster/clustering-" + str(i).zfill(5) + ".jsonl",
                "r",
                encoding="utf-8",
            ) as file:
                for line in file:
